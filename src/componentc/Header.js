...
import {isAuthenticated, signout} from '../page/auth/api';
function Header() {
        ...
        <div className="row custom-header align-items-center">
            ...
            <div className="col-lg-3">
                <ul className="d-flex justify-content-evenly">
                  ...
                  {isAuthenticated()&& (
                    <li className="list-unstyled"><Link to="/signin" className="text-decoration-none text-danger fs-5"
                    onClick={()=>signout(()=>{
                      navigate('/signin')
                    })}
                    ><i className="bi bi-box-arrow-right fs-3 "></i>Signout</Link></li>
                  )}
                    <li className="list-unstyled"><Link to="/cart" className="text-decoration-none text-white fs-5"><i className="bi bi-cart-plus-fill fs-3"></i></Link>Card</li>
                </ul>
            </div>
        </div>
    </div>
